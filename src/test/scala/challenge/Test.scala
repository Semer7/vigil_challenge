...
class Test extends AnyFunSuite with Matchers {
  ...
  test("Newlines should be preserved") {
    Main.reformat(testMultipleLines, exampleLength) shouldBe  exampleResultMultiline
  }

  test("Text is already formatted should return identical formatted text") {
    Main.reformat(exampleResult, exampleLength) shouldBe exampleResult
  }

  test("Example case with line limit 80") {
    Main.reformat(testOneLine, doubleLength) shouldBe resultDoubleLength
  }
}
